    ...
    $yhendus=mysqli_query($connection, $paring);
        if(mysqli_num_rows($yhendus)==1){
            if(mysqli_num_rows(mysqli_query($connection, "SELECT * FROM kasutajad WHERE nimi='$login' AND parool='$krypt' AND tyyp=1"))==1){
                ...
                echo "<script>self.location='https://mihhailov20.thkit.ee/phplehestik/content/kaubad/admin_index.php';</script>";
                exit();
            } else {
                $_SESSION['tuvastamine']='Tere kasutaja';

                echo "<script>self.location='https://mihhailov20.thkit.ee/phplehestik/content/kaubad/index.php';</script>";
                exit();
            }
        } else {
